...
function StatusBadge({ status, size = "sm" }) {
  ...
  return <span style={{ background: cfg.bg, color: cfg.color, borderRadius: "999px", padding: size === "lg" ? "8px 18px" : "5px 12px", fontSize: size === "lg" ? "13px" : "11px", fontWeight: 700, whiteSpace: "nowrap" }}>{cfg.label}</span>;
}
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 9999, background: "rgba(9,18,32,0.6)", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
      <div style={{ ...card, padding: "32px", width: "460px", maxWidth: "100%" }}>
        ...
        <div style={{ display: "flex", gap: "10px", marginTop: "18px", justifyContent: "flex-end" }}>
          ...
          <button onClick={() => reason.trim() && onConfirm(reason)} disabled={!reason.trim()} style={{ background: reason.trim() ? "#dc2626" : "#f1f5f9", color: reason.trim() ? "#fff" : "#94a3b8", border: "none", padding: "10px 20px", borderRadius: "999px", fontSize: "14px", fontWeight: 700, cursor: reason.trim() ? "pointer" : "not-allowed", fontFamily: "inherit" }}>Confirm Rejection</button>
        </div>
      </div>
    </div>
  );
...
function BookingDetailBody({ booking, resource, onApprove, onReject, onDelete }) {
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: "24px", alignItems: "start" }}>
      <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
        ...
        <div style={{ ...card, padding: "28px 32px" }}>
          <div style={{ fontSize: "12px", fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: "4px" }}>{resource?.type?.replaceAll("_", " ") || "Resource"}</div>
          <div style={{ fontSize: "24px", fontWeight: 800, color: "#0f172a", letterSpacing: "-0.5px", marginBottom: "6px" }}>{resource?.name || booking.resourceId}</div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
        {resource && (
          <div style={{ ...card, padding: "22px" }}>
            ...
            {(resource.imageUrl || resource.images?.[0]) && (
              <div style={{ borderRadius: "10px", overflow: "hidden", marginBottom: "14px", aspectRatio: "16/9" }}>
                <img src={resource.imageUrl || resource.images[0]} alt={resource.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={e => (e.target.style.display = "none")} />
              </div>
            )}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
              {[{ label: "Type", value: resource.type?.replaceAll("_", " ") || "—" }, { label: "Capacity", value: resource.capacity }, { label: "Location", value: resource.location || "—" }, { label: "Status", value: resource.status || "—" }].map(item => (
                <div key={item.label}>
                  <div style={{ fontSize: "10px", color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.6px", marginBottom: "3px" }}>{item.label}</div>
...
export default function BookingRequests() {
  ...
  const fetchData = async () => {
    try { setLoading(true); const [bRes, rRes] = await Promise.all([getAllBookings(), getAllResources()]); setBookings(bRes.data || []); setResources(rRes.data || []); }
  ...
  const counts = { ALL: bookings.length, PENDING: bookings.filter(b => b.status === "PENDING").length, APPROVED: bookings.filter(b => b.status === "APPROVED").length, REJECTED: bookings.filter(b => b.status === "REJECTED").length, CANCELLED: bookings.filter(b => b.status === "CANCELLED").length, CHECKED_IN: bookings.filter(b => b.status === "CHECKED_IN").length };
  const filtered = bookings.filter(b => statusFilter === "ALL" || b.status === statusFilter).filter(b => search === "" || b.userName?.toLowerCase().includes(search.toLowerCase()) || b.purpose?.toLowerCase().includes(search.toLowerCase()) || resources.find(r => r.id === b.resourceId)?.name?.toLowerCase().includes(search.toLowerCase()));

  const selectedResource = selected ? resources.find(r => r.id === selected.resourceId) : null;
  const selectedReadableId = selected ? generateBookingId(selected, selectedResource?.type) : "";
  const selectedImageUrl = selectedResource?.imageUrl || selectedResource?.images?.[0] || null;
        ...
        <section style={{ position: "relative", overflow: "hidden", background: "linear-gradient(90deg, rgba(9,18,32,0.96) 0%, rgba(15,41,71,0.88) 45%, rgba(22,58,99,0.78) 100%)", padding: "90px 2rem 50px" }}>
          ...
          <div style={{ ...container, position: "relative", zIndex: 2 }}>
            ...
            <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", padding: "7px 16px", borderRadius: "999px", background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.12)", color: "rgba(255,255,255,0.88)", fontSize: "12px", fontWeight: 600, marginBottom: "16px" }}>
              <span style={{ width: "8px", height: "8px", borderRadius: "50%", background: "var(--primary, #f4b400)", display: "inline-block" }} /> <b>BOOKING DETAILS</b>
            </div>

            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "20px" }}>
              <div>
               
                <h1 style={{ fontSize: "clamp(26px, 5vw, 46px)", fontWeight: 900, color: "#fff", letterSpacing: "-1px", marginBottom: "14px", lineHeight: 1.1 }}>
                  {selectedResource?.name || selected.resourceId}
                </h1>
                <div style={{ display: "flex", gap: "10px", flexWrap: "wrap", alignItems: "center" }}>
                  <span style={{ background: STATUS_CONFIG[selected.status]?.bg || "#f1f5f9", color: STATUS_CONFIG[selected.status]?.color || "#475569", borderRadius: "8px", padding: "6px 14px", fontSize: "12px", fontWeight: 800 }}>
                    {selected.status?.replace("_", " ")}
                  </span>
                  {selectedResource?.type && <span style={{ background: "rgba(255,255,255,0.12)", color: "rgba(255,255,255,0.85)", borderRadius: "8px", padding: "6px 14px", fontSize: "12px", fontWeight: 700 }}>{selectedResource.type.replaceAll("_", " ")}</span>}
                  {/* ✅ Booking ID badge in hero */}
                  <span style={{ background: "rgba(244,180,0,0.2)", color: "#f4b400", borderRadius: "8px", padding: "6px 14px", fontSize: "12px", fontWeight: 800, fontFamily: "monospace", letterSpacing: "1px", border: "1px solid rgba(244,180,0,0.3)" }}>
                    🔖 {selectedReadableId}
                  </span>
                  </div>
              </div>
              {selected.status === "APPROVED" && (
                <button onClick={() => downloadBookingConfirmation(selected, selectedResource?.name, selectedResource)}
                  style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(255,255,255,0.12)", color: "#fff", border: "1px solid rgba(255,255,255,0.22)", padding: "12px 20px", borderRadius: "999px", fontSize: "14px", fontWeight: 700, cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap" }}
        ...
        <section style={{ position: "relative", overflow: "hidden", background: "linear-gradient(90deg, rgba(9,18,32,0.96) 0%, rgba(15,41,71,0.88) 45%, rgba(22,58,99,0.78) 100%)", padding: "90px 2rem 70px" }}>
          ...
          <div style={{ ...container, position: "relative", zIndex: 2 }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: "10px", padding: "8px 18px", borderRadius: "999px", background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.12)", color: "rgba(255,255,255,0.88)", fontSize: "12px", fontWeight: 600, letterSpacing: "0.5px", marginBottom: "24px" }}>
              <span style={{ width: "8px", height: "8px", borderRadius: "50%", background: "var(--primary)", display: "inline-block" }} /> <b>BOOKING REQUESTS DASHBOARD</b>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: "24px" }}>
              <div>
                <h1 style={{ fontSize: "clamp(32px,5vw,56px)", lineHeight: 1.05, fontWeight: 800, color: "#fff", letterSpacing: "-1.5px", marginBottom: "14px" }}>Booking Requests</h1>
              ...
              </div> <div style={{ display: "flex", gap: "12px", flexWrap: "wrap", justifyContent: "flex-end" }}>
                  {[{ label: "Total", value: counts.ALL }, { label: "Pending", value: counts.PENDING }, { label: "Approved", value: counts.APPROVED }, { label: "Rejected", value: counts.REJECTED }, {label: "Cancelled", value: counts.CANCELLED}, {label: "Checked_In", value: counts.CHECKED_IN }].map(s => (
                    <div key={s.label} style={{ background: "rgba(213, 212, 212, 0.08)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: "16px", padding: "14px 20px", minWidth: "80px", textAlign: "center" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "12px", alignItems: "flex-end" }}>
                ...
                <div style={{ display: "flex", gap: "10px" }}>
                  <button onClick={() => navigate("/admin/checkin")} style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "var(--primary,#f4b400)", color: "#111827", border: "none", padding: "12px 20px", borderRadius: "999px", fontSize: "14px", fontWeight: 700, cursor: "pointer", fontFamily: "inherit", boxShadow: "0 8px 20px rgba(244,180,0,0.3)", whiteSpace: "nowrap" }}>📷 Check-In Scanner</button>
                  <button onClick={() => downloadBookingSummaryReport(bookings, resources)} style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(255,255,255,0.12)", color: "#fff", border: "1px solid rgba(255,255,255,0.22)", padding: "12px 20px", borderRadius: "999px", fontSize: "14px", fontWeight: 700, cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap" }}
                    onMouseEnter={e => (e.currentTarget.style.background = "rgba(255,255,255,0.20)")} onMouseLeave={e => (e.currentTarget.style.background = "rgba(255,255,255,0.12)")}>↓ Download Report </button>
                </div>
              </div>
            </div>
          </div>
      ...
      <div style={{ ...container, marginTop: selected ? "32px" : "-32px", position: "relative", zIndex: 3, paddingTop: 0, paddingBottom: "40px" }}>
        {selected ? (
          ...
          <>
            ...
            {loading ? <div style={{ ...card, padding: "60px", textAlign: "center", color: "#94a3b8", fontSize: "15px" }}>Loading...</div>
              : filtered.length === 0 ? <div style={{ ...card, padding: "60px", textAlign: "center" }}><div style={{ fontSize: "38px", marginBottom: "12px" }}>📋</div><div style={{ fontSize: "18px", fontWeight: 700, color: "#0f172a" }}>No bookings found</div></div>
              : <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>{filtered.map(b => <BookingListCard key={b.id} booking={b} resource={resources.find(r => r.id === b.resourceId)} onClick={() => setSelected(b)} />)}</div>}
          </>
        )}
      </div>
