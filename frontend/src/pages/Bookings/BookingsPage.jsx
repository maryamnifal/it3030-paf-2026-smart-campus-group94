...
import { downloadBookingConfirmation } from "../../utils/bookingPDF";
...
const STATUS_CONFIG = {
  ...
  CHECKED_IN: { label: "Checked In", bg: "#dbeafe", color: "#1e40af" }, // ✅ NEW status for checked-in bookings
};
...
function BookingForm({ onSuccess, onCancel }) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await createBooking({ ...form, userId: currentUser, userName: currentUserName || currentUser, startTime: form.startTime + ":00", endTime: form.endTime + ":00", expectedAttendees: parseInt(form.expectedAttendees, 10) });
      onSuccess();
    } catch (e) {
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1.05fr 0.95fr", gap: "28px", alignItems: "start" }}>
      <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
        <div style={{ ...pageCardStyle, padding: "30px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(430px, 1fr))", gap: "24px",alignItems: "start"}}> 
            <div style={{ gridColumn: "1 / -1" }}>
              ...
              {!loadingResources && resources.length === 0 && (
                <span style={{ fontSize: "12px", color: "#991b1b", marginTop: "6px", display: "block" }}>No active resources found. Please add resources in the Facilities module first.</span>
              )}
            </div>

            <div style={{ gridColumn: "1 / -1" }}>
              <label style={labelStyle}>Date *</label>
              <input type="date" value={form.date} onChange={(e) => { set("date", e.target.value); setSelectedSlot(null); }} min={new Date().toISOString().split("T")[0]} style={inputStyle} />
            </div>

            <div style={{ gridColumn: "1 / -1" }}>
              <TimeSlotSelector resource={selected} selectedDate={form.date} selectedSlot={selectedSlot} onSlotSelect={handleSlotSelect} />
            </div>
            ...
            <div style={{ gridColumn: "1 / -1" }}>
              ...
              {selected?.capacity && (
                <span style={{ fontSize: "12px", color: "var(--text-muted)", marginTop: "6px", display: "block" }}>Max capacity for this resource: {selected.capacity}</span>
              )}
            </div>
          </div>
        </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
        ...
        <div style={{ ...pageCardStyle, padding: "30px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
            {["Bookings require admin approval before confirmation.", "Only ACTIVE resources are shown in the dropdown.", "You can cancel a pending or approved booking anytime.", "Rejected bookings will show a reason from the admin.", "Approved bookings include a QR code for check-in."].map((tip) => (
        ...
        <div style={{ ...pageCardStyle, padding: "24px" }}>
          <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
            <button
              ...
              style={{ background: "var(--primary)", color: "#111827", border: "none", padding: "14px 24px", borderRadius: "999px", fontSize: "14px", fontWeight: 700, cursor: loading ? "not-allowed" : "pointer", boxShadow: "0 10px 24px rgba(244,180,0,0.22)", transition: "all 0.25s ease", opacity: loading ? 0.7 : 1 }}
            >
              {loading ? "Submitting..." : "Submit Booking Request"}
            </button>
            <button
              onClick={onCancel}
              style={{ background: "#fff", color: "var(--text-dark)", border: "1px solid rgba(15,23,42,0.08)", padding: "14px 24px", borderRadius: "999px", fontSize: "14px", fontWeight: 600, cursor: "pointer", transition: "all 0.25s ease" }}
    ...
    <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999 }} onClick={onClose}>
      <div style={{ background: "#fff", borderRadius: "20px", padding: "32px", maxWidth: "600px", width: "90%", maxHeight: "80vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.3)" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px", marginBottom: "20px" }}>
          ...
          <div>
            <label style={labelStyle}>Date *</label>
            <input type="date" value={form.date} onChange={(e) => { set("date", e.target.value); setSelectedSlot({ start: "", end: "" }); }} style={inputStyle} />
          </div>
          <div style={{ gridColumn: "1 / -1" }}>
            <TimeSlotSelector resource={selected} selectedDate={form.date} selectedSlot={selectedSlot} onSlotSelect={handleSlotSelect} />
          </div>
          <div style={{ gridColumn: "1 / -1" }}>
            <label style={labelStyle}>Purpose *</label>
            <input type="text" value={form.purpose} onChange={(e) => set("purpose", e.target.value)} style={inputStyle} />
          </div>
          <div style={{ gridColumn: "1 / -1" }}>
            <label style={labelStyle}>Expected Attendees *</label>
        ...
        <div style={{ display: "flex", gap: "12px", justifyContent: "flex-end" }}>
          <button onClick={onClose} style={{ background: "#f3f4f6", color: "var(--text-dark)", border: "none", padding: "12px 24px", borderRadius: "999px", fontSize: "14px", fontWeight: 600, cursor: "pointer" }} onMouseEnter={(e) => (e.target.style.background = "#e5e7eb")} onMouseLeave={(e) => (e.target.style.background = "#f3f4f6")}>Cancel</button>
...
// ✅ BookingCard now includes QRCodeDisplay for APPROVED bookings
    ...
    >
      ...
      <div style={{ padding: "22px" }}>
        ...
        {booking.status === "APPROVED" && (
          <button
            onClick={() => {
              const res = resource; // already passed as prop
              downloadBookingConfirmation(booking, res?.name, res);
            }}
            style={{
              background: "#fff",
              color: "#0f172a",
        ...
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginTop: "14px" }}>
          ...
          {canEdit && (
            <button
              ...
              style={{ background: "#fff", color: "var(--primary)", border: "1px solid rgba(244,180,0,0.3)", padding: "10px 18px", borderRadius: "999px", fontSize: "13px", fontWeight: 600, cursor: "pointer", transition: "all 0.2s" }}
              onMouseEnter={(e) => (e.target.style.background = "rgba(244,180,0,0.1)")}
          ...
          {canCancel && (
            <button
              ...
              style={{ background: "#fff", color: "#dc2626", border: "1px solid rgba(220,38,38,0.15)", padding: "10px 18px", borderRadius: "999px", fontSize: "13px", fontWeight: 600, cursor: "pointer", transition: "all 0.2s" }}
          ...
          {/* ✅ QR Code button — only shows for APPROVED bookings */}
        {booking.status === "APPROVED" && (
...
export default function BookingsPage() {
  ...
  filtered = filtered.filter((b) => {
    ...
    return (
      (!searchQuery || (b.id || "").toLowerCase().includes(q) || (resource?.name || "").toLowerCase().includes(q) || (b.purpose || "").toLowerCase().includes(q)) &&
      ...
      <section style={{ maxWidth: "1200px", margin: "0 auto", padding: "40px 2rem 0" }}>
        ...
        <div style={{ ...pageCardStyle, padding: "28px" }}>
          ...
          <div style={{ display: "flex", gap: "16px", marginBottom: "24px", flexWrap: "wrap" }}>
            {[{ label: "Total Bookings", value: counts.ALL, color: "#9ca3af" }, { label: "Pending", value: counts.PENDING, color: "#fbbf24" }, { label: "Approved", value: counts.APPROVED, color: "#15803d" }, { label: "Rejected", value: counts.REJECTED, color: "#b30404" }].map((s) => (
          ...
          {loading ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "20px" }}>
              {filtered.map((b) => (
                <BookingCard key={b.id} booking={b} onCancel={handleCancel} onEdit={setEditBooking} resources={resources} />
              ))}
            </div>
          )}
        </div>
